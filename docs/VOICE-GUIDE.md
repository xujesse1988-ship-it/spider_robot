...
| 前进 / 往前走 / 后退 / 倒退 | 前后走。**不带时长=一直走**（回复念"一直前进，说停就停"），喊停/新指令/键盘为止 |
| 左移 / 右移 / 向左走 / 往右挪 | 平移（连续语义同上） |
| 左转 / 右转 / 向左拐 / 顺时针 / 掉头 | 原地转（连续语义同上） |
| … 三秒 / 5 秒 / 两步 / 半分钟 | 带时长则到点自停；上限 10 s（`--max-secs`，防把时长听歪）；步 = 1 s |
| 快点 … / 慢点 … | 本句速度 ×1.5 / ×0.6（"快点前进"） |
| 快点 / 慢点（单说） | 调速：持续倍率 ×1.5/×0.6 夹 0.4~2.0，走动中立即生效、后续指令继承；"太快了"=减速；到顶回"已经最快了"。键盘键不跟倍率 |
...
python scripts/voice_teleop.py --vent off          # 阀策略对照（默认 auto：站起/走动时六阀通电排气让吸盘通大气、
                                                   # 站着不动断电；on 常通；off 不碰阀=装气路后被动真空锁脚抬不起；
                                                   # v 键运行中轮换，同 walk_teleop）
```
...
```bash
python scripts/voice_enroll.py            # 读 5 段提示语 + 4 个短词注册 → voiceprint_owner.npz
python scripts/voice_enroll.py --test     # 自己验一次（✓），换别人验一次（✗）
python scripts/voice_enroll.py --append   # 已有档案只补录短词（阈值不动，不用重录）
...
- **短词锚点**（09-03 加）：声纹模型对 <1 s 音频受说话内容影响明显——同一个人
  说"确认"对着"前进三秒"的注册段，分数就是低。注册流程末尾会让你逐个读
  确定/确认/退出/好的 四个高频短词，裁出人声区间单独存锚点；实战说这个词就有
  逐词对上的参照。急停词不补（急停永不做声纹）。已有档案跑 `--append` 即可，
  不用整套重录。开发机量化：主人短词 0.31~0.50 → **0.50~0.69**（全过线）；
  陌生人说同词分数也会涨约 0.1（内容对上了），24 个合成声音 × 4 词共 96 次
  最高只到 0.412，仍低于 0.45 折让线，没混过的。
- 误拒（自己被拒，看日志里的分数和秒数）→ 先 `--append` 补录被拒的那个词 /
  阈值调低 / 重新注册（注册环境要和使用时接近，泵开着用就开着泵注册几段）；
  误纳（别人能指挥）→ 阈值调高。
...
### 3.9 爬墙语音壳（`scripts/voice_climb.py`）

给 `climb_walk.py`（P4 爬墙联调）加语音，但架构与 voice_teleop 不同：
**climb_walk 一行不改，原样跑在 pty 里**，语音识别出的指令映射成按键写进
pty——与手敲完全等价，它的全部互锁（单步在途/已放开/大步幅禁侧移/冻结）
照常生效、按键照常进黑匣子；真键盘 raw 透传（ESC×2/o×2/Ctrl-C 语义不变）；
它的关键输出（就位/吸附完成/冻结/悬停/单步完成/已放开）由壳念出来。
语音引擎挂掉不影响爬墙（键盘照常）。

| 说 | 键 | 说明 |
|---|---|---|
| 停下 / 停止 / 别动 | 空格 | 急停，不用唤醒、谁喊都停、打断说话 |
| 前进/后退/左移/右移/左转/右转 [N 秒] | w/s/a/d/q/e | 不带时长=一直走；带时长到点壳补空格自停（上限 `--max-secs` 默认 30）；吸附完成前不注入 |
| 单步 / 抬腿；落地 / 踩下 | i | 抬/落两拍都是 i，phase 由 climb_walk 判 |
| 解冻 / 解除冻结（认"冻结"就够） | f | f 在非冻结态是空操作，词表放宽安全 |
| 开始吸附 / **启动** | p | 就位暂停后启动全吸附序列；"吸附"易被听歪（实测 戏附/洗服/吸服 已收编），"启动"发音硬朗最稳 |
| 电压 | — | 念壳从状态行提取的电压/电流/最差盘压 |

**语音黑名单**（永远只认键盘——听歪一个词就放气的险不能冒）：退出
（ESC×2，会放气，墙上=坠落）、放吸盘取机（o×2）。语音说"退出/取机"只回
键盘指引。快点/慢点无效（速度 `--speed` 定死）；站立/趴下/换步态不支持。
另有第二道回声守卫：动作词（走/单步/落地/解冻/启动）若包含在机器人 2.5s
内自己说过的话里则不执行——引擎层回声过滤对 ≤2 字文本只认全等（保真急停），
"悬停中，说落地收口"的回声在 --trust-aec 下可能被听成"落地"穿过第一道。

用法：`python scripts/voice_climb.py [语音参数] [climb_walk 参数原样透传]`
（两边参数名无冲突；`--release` 直接转交 climb_walk 不起语音）。声纹锁有
档案自动开（急停不拦）。播报措辞纪律同 §3.3：不得含 KWS 急停词。
...
| **走路时要喊很大声才听见** | 09-03 实录定案（§5 第九轮）：走动噪声 98% 在 >2kHz（齿轮啸叫），语音带内信噪比仅 ~2dB；**短板是 KWS 3.3M 小模型，ASR 常常听得懂**（同段录音 ASR 出"前进3秒"全对、KWS 唤醒要 3.0:0.10 才命中=超急停纪律不取）。滤波已证伪：高通白搭（低频只占 1%）、低通/带限连清辅音一起砍 ASR 更糊。**走动会话解法：`--no-wake` + 声纹锁**——纯舵机噪声 VAD 零切句不会乱出指令（实测），陌生人被声纹拒，急停 KWS 照常。想留唤醒模式：喊"蜘蛛同学"（音节结构比"小蜘蛛"抗噪，"小"的 x 清辅音正泡在噪声带里）。物理=挪远+麦克风朝人+与舵机间加泡棉挡板（高频方向性强衰减快；减振没用）。工具：`--noise-test` 引导测量、`--noise-analyze` 对已录 wav 分频段。录音增益无软件旋钮（固件不暴露 amixer 控制项）；`HEXAPOD_AUDIO_HPF` 高通留给以后泵类低频噪声用 |
...
第七轮（09-03 实机反馈"确认/退出还是不准"）：多尺度子窗只缓解了长短失配，
0.5 s 短词还剩"说话内容不同"这层——声纹模型对 <1 s 音频受内容影响明显，同一个
人的"确认"对着"前进三秒"的注册段分数就是低。修=注册流程末尾逐个补录
确定/确认/退出/好的 四个短词（裁人声区间存同词锚点），已有档案 `--append` 追加
不用重录。开发机量化（vits-zh-ll sid0 当主人；同引擎 4 声 + aishell3 真人语料
20 声当陌生人，测试音加 20 dB 底噪、变速避开与锚点同波形）：主人短词 A 档
0.31~0.50 → B 档 **0.50~0.69** 全过线，没录过锚点的短词（退下/算了）不受影响；
陌生人说同词分数涨约 0.1（内容对上），96 人次最高 0.412 仍在 0.45 折让线下，
0 混过；陌生人长指令最高 0.400。pytest 185 例全绿。

第八轮（09-03 voice_climb 开发机全链路，KWS+SenseVoice 真模型 + 合成指令
音轨驱动 climb_walk --mock）：见证=唤醒→"现在启动"→键 p→吸附序列→"六足
吸附完成"播报→"往前走"→键 w→状态行速 +13（真走）→KWS 急停→空格→键盘
ESC×2 完整退出序列→退出码 0；吸附完成前说行走=拒绝不注入、声纹拒绝、
就位暂停中止路径、eof 后键盘接管均见证。过程战果：①"电压→低压""解除→
接出""吸附→戏附/洗服/吸服"等高频听歪已收编词表；②合成音轨的坑（与实机
无关但记录备查）：vits-zh-ll 念两字孤词发音失真（"停下"念成"因子"），短词
必须放载体句；同一段"小蜘蛛"音频跟在不同上下文后 KWS 得分贴阈值线，
num_threads>1 数值抖动会翻车——实机人声无此现象，急停/唤醒此前实测正常。
pytest 189 例全绿。

第九轮（09-03 "走路要喊很大声"实录归因，用户上传三段 --noise-test 录音在
开发机全套分析）：①频谱定性——走动噪声 98% 在 >2kHz（齿轮啸叫）、<300Hz
仅 1%，"结构传导低频"假设被推翻，高通证伪（收益 0.0dB）；语音带（300~4k）
内信噪比仅 ~2.4dB，但 2kHz 以下用户语音干净（信噪比 17dB+）。②滤波证伪
——低通 3400/2500、带限 250-3400 三个变体过整链：ASR 反而更糊（"小蜘蛛
前进3秒"→"四周/这周/都前进3秒"），清辅音与噪声共享高频带，砍噪声必砍
辅音，"过滤人声频段"这条路收口。③真短板=KWS：同一段带噪实录，ASR 直接
出"饺蜘蛛前进3秒"（指令全对），KWS 唤醒阈值扫到 3.0:0.10 才命中——超过
急停 2.0 的纪律上限，不取。④解法实证：--no-wake 下纯走动噪声 VAD 零切句
（不会乱出指令）、静止段唤醒+指令全对——**走动会话推荐 --no-wake+声纹锁**；
Highpass（HEXAPOD_AUDIO_HPF，默认关）留给以后泵类低频噪声；
--noise-analyze 分频段工具落地并按三种噪声形态给判读。pytest 192 例全绿。

## 6. 待办

- [x] 插板跑 `voice_setup.sh` + `voice_check.py`（09-02 通过，卡名 `Lite`，见 §5）
- [ ] 日常使用中若调过 KWS 阈值/混音器音量，回填 §3.6
- [ ] 09-03 走路听不清已归因（§5 第九轮：KWS 是短板、滤波证伪）。实机待验：
  ①走动会话 `voice_teleop --no-wake` 实跑（声纹锁把门；纯噪声不乱出指令已
  实证），记误动作/漏指令；②唤醒模式下"蜘蛛同学"vs"小蜘蛛"走动命中对比；
  ③物理调整（麦克风挪远/朝人/与舵机间泡棉挡板）后重跑 --noise-test，目标
  语音带内信噪比 >10dB 回填。词表 0.20:1.5 已放灵敏，拉取后仍需重新生成
  keywords_hexapod.txt（静止/低噪时有效）
...
- [ ] 声纹锁实机：`voice_enroll.py` 注册（含开泵环境段）→ `--test` 本人/他人各验
  ...
  09-03 拉取后先 `--append` 补录短词（或重新注册，流程已带短词环节），
  再用 `--test` 说"确认""退出"看分数是否 ≥0.45
- [ ] 舵机/泵噪声下的识别率实测；必要时换 `sherpa-onnx-kws-zipformer-zh-en-3M-2025-12-20`
- [x] ~~`climb_walk.py` 接语音急停事件~~（09-03 `voice_climb.py` 语音壳落地，
  急停+行走+单步+解冻+启动全套语音，§3.9；开发机全链路见证，待实机）
