...
| The whole robot dies at startup after the valve coils light up one by one (LED green → solid red, SSH drops, intermittent) | **Settled on the real robot 09-03 (lean_20260903_135325.log): the trigger is the servo relay closing, not the valves** — while the six coils were energized one by one, the 5V stayed flat at 4.88–4.89V with thr=0x0; `servo relay closed` (`舵机继电器已合闸`) is the last line, and it died <50ms later. Solid red = the halt state where the Pi 5's PMIC has cut power to the SoC (same as after `halt`; it needs a power cycle). The root cause is margin: the Pi's 5V input is only 4.89V unloaded (a healthy buck should give 5.05–5.15; `get_throttled=0` cannot show this kind of "not over the line but no margin left"), and 18 servos powering up at the same instant tug the shared 2S battery over the line. The root fix is in the supply: measure under load at the Pi's USB-C end and tune the buck output to 5.1–5.2V; swap in a short thick USB-C cable; put ≥1000µF electrolytics across both the buck's input and its output; verify that the servo return ground goes straight back to the battery negative and does not detour through the Servo2040's USB ground. Software mitigation (done): `enable(True)` is split in two — close the relay (powered, no torque) → 0.4s → firmware enable (torque), with the black box sampling the bus in each segment (`合闸后 x s` / `使能后 x s`, "x s after relay closure" / "x s after enable"), so the next death can be told apart as an inrush from powering up versus from producing torque. Forensics tools: run `bash scripts/pi_forensics.sh setup` once (it makes kernel logs persistent and must be run **before** the crash), then power-cycle after a reproduction and run `pi_forensics.sh check`; the black box writes `TLM 电源 5V=…` ("TLM power 5V=…") every 0.1s (`vcgencmd pmic_read_adc`; the user must be in the video group). The Pi 5 has no RTC battery, so the start time in the boot list is a fake clock saved last time (it can be off by hours); `check` back-computes the real boot time from the true timestamp of the last log line. **Added 09-04**: after the buck was swapped for an adjustable module set to 5.19V (the PMIC reads 5.105V, already at the level of the official supply), walk_teleop still died at startup (walk_teleop had no black box then; it has since been added, tag=walk/voice) — the static voltage is not the root cause; it is a sub-millisecond dip / ground bounce at the instant of relay closure, invisible to 10Hz sampling and to a multimeter; `/proc/device-tree/chosen/power/power_reset`=2 was read both times and was taken, per the Raspberry Pi engineers, as the PMIC's own testimony that the previous boot was cut for low voltage; **control on 09-06: it still reads 2 after several normal poweroffs/reboots, so on this machine it has no discriminating power and does not count as evidence** (the solid red LED after death already says the PMIC cut the SoC; a kernel crash does not turn the LED red). The battery is an unprotected 2S RC LiPo with no protection board, so there is no over-current trip and battery sag is not the prime suspect; **verdict on the evening of 09-04 (case file `html/en/pi-crash-ground-loop-20260904.html`)**: ① the user found that every walk_teleop run turns the sticky bits from 0x0 to 0x50000, even when nothing dies — the intermittent fatal event now has a deterministic non-fatal stand-in, so the experimental criterion becomes "do the bits get set or not"; ② walk_20260904_215456.log: 14ms after GPIO17 went high, thr=0x50005, with the servo bus at 0.00A at that moment; after enable it drew 6.67A and the battery went 7.71→7.31V while the Pi's 5V didn't budge — it is not the servos' large current, it is the instant of relay closure; ③ an A/B within a single boot, closing the relay with the GPIO17 script only: with the Pi↔Servo2040 USB cable unplugged, ten closures all gave 0x0; plugged back in, the very first one gave 0x50000 — **the path is the USB ground loop**; ④ the new buck is an XL4015 5A CC/CV board whose back-side R050 low-side sense resistor sits in the Pi's ground return, and its blue LED flashing green at the instant of closure = the module itself also sees this pulse (an aggravating factor; the old fixed module had no such resistor and died too, so it is not the root cause). Mechanism: the contacts closing charge the capacitance of 18 servos + the Servo2040 with a hundred-microsecond, tens-of-amps pulse that creates a voltage difference along the servo return ground; part of it flows through the USB ground → the Pi's ground plane → the GND Dupont jumper → R050 back to the battery negative, and the drop across the jumper + R050 looks to the PMIC exactly like a 5V dip. Remedies (not installed yet; the criterion is still the sticky bits): a USB isolator (full-speed, ADuM3160 class) is the cleanest; pre-charge / soft start on the servo rail (a small relay in series with 4.7Ω 5W closing 100ms first, or a P-MOS soft start; a manual pre-charge with 10Ω across the contacts can verify it first); a star ground can only reduce it. A capacitor across the Pi's 5V is only symptomatic (2200µF at 0.7A holds 0.4V for about 1ms). **Sequel 09-06 (case file section 08)**: with an ADuM3160 isolator fitted, ten GPIO17 closures all read 0x0, so the USB path is confirmed cut, yet body_lean still died tens of ms after closure (lean_20260906_234509.log ends at "relay closed", the 0.05s post-closure sample never came); island test: power off, disconnect the buck's IN−, Pi GND to battery negative reads open → the Pi side reaches the battery side through the buck alone, so the ground loop is not the only entry (YYNMOS-8 IN−/DC− optocoupler isolation was already measured on the pneumatic-electrical page); the remaining entry is the buck itself: the XL4015 module needs input ≥ output + 1.5V = 6.7V, the 2S pack sits at 7.70V under the valve load / 7.31V with servos enabled, under 1V of margin, and the closure pulse (30A × 30mΩ ≈ 0.9V and up) pulls it under; closure alone (valves off) is clean ten times while the real scripts (six valves energized) die, the difference being exactly that the valve load pulls the battery down first; remedy = replace the buck with a low-dropout synchronous buck (holds 5.1V with input a little over 6V, ≥5A, dropout <0.5V) or a buck-boost, keep the isolator, pre-charge demoted to secondary, input capacitor + Schottky not recommended; after the swap run the real scripts several times, criterion = no sticky bit + no death; **09-07 software stopgap**: all four startup scripts gained `--relay-first`, which closes the relay as soon as the serial port is open (valve coils not yet energized, the battery carries only the Pi) while firmware enable stays after the valves; run with it before the buck swap, and it doubles as an A/B: if it still dies with the relay first, "the valve load eats the margin" does not hold; **09-07 result**: the first relay-first run (body_lean --no-tank) died tens of ms after closure (last line 0.452s "relay closed", valve coils not energized, 5V 5.101V, serial port opened 0.25s earlier), "the valve load eats the margin" is withdrawn, trigger = closure itself; after the isolator the closure-only script died 1 in 11, the real scripts 3 in 3, remaining differences = serial port open + the Pi's own load (power-watch subprocesses / fsync per line); next: power the Pi from a power bank and run three times to settle it (bus readings come from the Servo2040 over serial, the Pi side can float with no ground wire): alive = the entry is the Pi's 5V source → buy the low-dropout synchronous buck, dies = unrelated to conducted power → prime suspect the serial link, then the closure-only script with the serial port open as a control; **located on the evening of 09-07**: the Pi moved to a power bank (the relay module's coil side DC+/DC− must move to the Pi header 5V/GND; on the buck OUT± the IN1 signal has no return path and the relay does not move), nothing else changed, closure no longer kills it (run count / sticky-bit readings to be added) → entry = the Pi's 5V source (XL4015 with no margin on 2S), the serial link and the Pi's own load are cleared; the mechanism (pulse pulls the battery → buck input below 6.7V → output collapses → PMIC cuts) is still inference, confirmed only by a scope on the buck input and output together; remedy = low-dropout synchronous buck / buck-boost, or a small separate battery for the Pi; the conclusion has been revised four times (power_reset withdrawn, XL4015 8V → 1.5V headroom, one path → two entries, valve load withdrawn), each by a control experiment, and the present one likewise awaits a scope check; **full-battery control, evening of 09-07**: battery fully charged, Pi back on the XL4015, nothing else changed, a real script stood up and get_throttled=0x0 (not even "undervoltage occurred") — same buck, three deaths in three at 7.3~7.9V versus 0x0 on a full battery pins the entry to the battery → buck → Pi path and clears the serial link; the power-bank run is confirmed: relay coil side moved to the Pi header 5V/GND (not one wire between the Pi side and the battery side), a real script was run and did not die, run count / sticky bit not recorded; after reconnecting, the Pi's 5V/GND and the relay DC+/DC− go together to the buck OUT±; a full battery is not a remedy, one session drops it back to 7.6V and into the zone where it dies, so before the swap measure the battery before powering up and do not start below 8.0V (withdrawn 09-08, see below); **09-08**: with the battery measured at 8.0V (user's meter; when it was read is not recorded), mount_wall on the real full chain died at closure again (mount_20260908_225510.log ends at 1.329s "relay closed", 5V flat at 5.10V / thr=0x0 through all six valve coils, the journal stops at 909s after boot with the black box starting at 2169s, wtmp has a reboot and no shutdown; same signature; after the isolator the real scripts stand at 4 dead / 4, plus one survival on a full battery) — "do not start below 8.0V" is withdrawn (the line was extrapolated from one survival whose voltage was never measured), and "battery voltage is the controlling variable" drops back to a single sample (the closure-only script survived 10 of 11 on the same buck, so the event is probabilistic and one survival cannot tell voltage from luck); the entry's location now rests on the power-bank group alone (runs not counted; the real scripts die about four times in five, so one survival by chance is one in five); next = Pi on a power bank with the relay coil on the Pi header, a real script five times in a row with get_throttled recorded each time, all alive and 0x0 places the entry on the 5V source and clears the way to replace the buck or give the Pi its own battery, one death voids the buck reading and the next control is the closure-only script + serial port open; until the count is in, do not run real scripts on the XL4015 (every death = an unclean power cut to the SD card) |
