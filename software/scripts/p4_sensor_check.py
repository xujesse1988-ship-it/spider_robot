...
⚠⚠ **但绝不能和 mount_wall.py / climb_walk.py 同时跑**（09-12 实机踩过）。
"不碰 GPIO" 说的是不驱动硬件，不等于可以并发：ADS1115 的单次转换是
"写配置寄存器选通道并启动 → 读转换寄存器"两步，两个进程在同一条总线上一交错，
**谁都可能读到对方那个通道的值**。而爬墙/过渡引擎的**泵滞环与漏气监护就吃这些读数**，
于是表现为"泵不到一秒抽一次"（正常是 6~8 秒一次），像极了大漏气。
09-12 E6 实机就是这个症状，关掉本脚本立刻恢复正常。
⇒ 引擎在跑的时候，气压看它自己状态行上的 `盘差` 和各腿 `@-xx`，别开第二个窗口。
