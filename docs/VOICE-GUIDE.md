...
- [ ] 09-03 唤醒词放灵敏（0.20:1.5，起因"走路时要喊很大声"）：拉取后在树莓派
  **重新生成 keywords_hexapod.txt**（命令见 §3.3 第 2 条）→ 走动中实测唤醒
  距离/音量 + 误唤醒次数回填；先 `voice_check.py --noise-test` 拿信噪比与
  AGC 判读，物理安装（远离舵机/减振/朝向）按判读调
