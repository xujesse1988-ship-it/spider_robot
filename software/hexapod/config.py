...
class LegConfig:
    ...
    # vent 前零力交接量 δ mm（docs/HANDOVER-DESIGN.md；0=关，默认关）。
    # 08-20 原地踏步量化（html/vent-snap-20260820.html）：83% 的下滑发生在
    # 放气密封破裂一瞬——被抬腿吸附期间攒下的弹性势能一步释放。窗头决策后
    # 先进 HANDOVER：本腿指令沿"上坡"还 δ（卸载到 ~0 力），其余支撑腿各沿
    # "下坡"接 δ/5（均值不变=身体指令不动），铺完才放气（climb.py 4.7 步）。
    # 逐腿字段：实测单次弹跳逐腿差 4 倍（L1 20.9 vs L2 5.8mm），全局一个值
    # 没法用；起标=实测弹跳×0.8（宁欠勿过，A/B 逐次加）
    handover_mm: float = 0.0
    # 逐腿吸盘轴垂直度修正角°，叠加在全局 cup_tilt_trim_deg 上（09-13：站立时主要中腿吸盘不垂直、
    # 要往身体收——各腿带载让位不同，一个全局数兜不住）。正值 = 这条腿在模型"垂直站位"上吸盘轴仍向外
    # 倾这么多度；站位半径、v 键最正点、落点带随之内收（每 1° 约 2 mm）。mount_wall --tilt-trim L2:6,R2:6
...
DEFAULT_LEGS = (
    _leg("L1",  83.5,  63.0,  55.0, (15, 16, 17), 23,
         coxa_cal=ServoCal(channel=15, attach_deg=-4.31, us_m45=1959.0, us_p45=1041.0),
         femur_cal=ServoCal(channel=16, attach_deg=48.71, us_m45=1973.4, us_p45=1026.6),
         tibia_cal=ServoCal(channel=17, attach_deg=96.06, us_m45=1947.3, us_p45=1052.7)),
    ...
    _leg("R1",  83.5, -63.0, -55.0, (12, 13, 14), 22,
         coxa_cal=ServoCal(channel=12, attach_deg=0.38, us_m45=1954.9, us_p45=1045.1),
         femur_cal=ServoCal(channel=13, attach_deg=54.2, us_m45=1043.2, us_p45=1956.8),
...
class RobotConfig:
    ...
    # 吸盘轴垂直度整机实测修正角°（L3-DISPUTE 'g' 项下游标定钩子）：正值 =
    # 实机在模型算的"垂直站位"上吸盘轴仍向外倾这么多度（来源：标定残差 +
    # 带载腿链让位 + cup_delta 自由态口径与带载姿态的偏差——分不清也不必分，
    # 一个总修正角全兜住）。生效方式 = 并入 _press_tilt 的轴向角：站位半径、
    # 落点带、直行步幅上限全部随之整体内收（每 1° 站位约收 2mm）。08-19 地面
    # 实测吸盘轴不垂直、落点内收更垂直，climb_walk --tilt-trim 在线试值标定
    cup_tilt_trim_deg: float = 0.0
    ...
    handover_slot_w: tuple = ()      # 零力交接载荷分配权重（5 元；空=均分）。
                                    # 按窗序轮转距离套用：w[0]=最晚才轮到抬的
                                    # 支撑腿（刚抬过的），w[4]=下一个要抬的。
                                    # 晚轮到的多接——每次落地的零预载锁定会把
                                    # 受力格局重摊，早收到的载荷被后续落地稀释
                                    # 回集体、晚收到的全额攒到自己抬腿
                                    # （HANDOVER-DESIGN 附录 A 模型：单足激进档
                                    # 0.6/0.25/0.1/0.05/0 稳态循环内应力 -32%；
                                    # 非单调，反向 +30%）。双足（CLIMB_DUAL）
                                    # 份额=按距离取值后就地归一（climb._share_
                                    # now），档位按口径分家：双足推荐 1,1,1,0,0
                                    # （模型最差单腿 -12%/均值 -21%），单足档
                                    # 照搬双足会把对内后腿推高过均分（DUAL-
                                    # SWING-DESIGN §3.5/附录 C）。⚠ 排序按窗序
                                    # 轮转距离不按落地新旧（不依赖吸附历史；
                                    # 启动吸附序现也=窗序）；口径假设按轮转
                                    # 抬腿，偏离轮转的抬腿由引擎守卫退回均分。
                                    # ⚠ 开权重改变稳态运行点，δ 表按开权重
                                    # 工况标
    handover_rate_mms: float = 10.0  # 零力交接铺设速率 mm/s（climb.py 4.7 步；
                                    # 单源默认，引擎导出别名 HANDOVER_SPEED_MMS
                                    # 供测试/文案时长账）。吸住的脚改指令=改力，
                                    # 载荷重分配要留准静态时间——10 与 LEAN_
                                    # SPEED 同量级，是 08-25/26 n=3 数据的基线
                                    # 口径。T1 拟合（html/stiffness-fit-
                                    # 20260826.html）：交接段 ~九成是"共模下沉
                                    # ≈蠕变率×窗时长"，而窗时长≈0.5+δ/rate 与
                                    # δ 完全共线——D′ 判别实验=改 20 一次一变
                                    # 量：下沉按比例降=时间蠕变（预测 C 交接段
                                    # 39.7→~25mm），不降=逐 mm 损耗（模型改
                                    # 写），两个结果都进论文（协议 §9）。引擎
                                    # 守卫 0<rate≤50（50=5×默认，理智护栏非物
    ...
    lift_gate_kpa: float = -50.0    # 抬腿门槛：窗头放行前其余 5 支撑盘压须全部
                                    # 深于此值（0=关）。ATTACHED 的 -30 只是
                                    # "密封住"判据；窗序首尾相接，每腿都在前一
                                    # 腿刚过 -30 的瞬间开抬，越到周期后段软盘越
                                    # 多（08-19 实测 w 模式 R2 刚过线 R3 就抬，
                                    # R2/R3 抬腿整机下坠，L 侧起步时支撑还深所
                                    # 以无感）。-50 取攻守中点：泵滞环反正要拽
                                    # 到 -75，等这一脚是顺路。air 模式旁路
    lift_gate_timeout_s: float = 15.0  # 门槛等待上限 s，超时冻结报警（点名压不
                                    # 下去的腿）：无罐+唇口微漏可能永远到不了
                                    # 门槛，不能静默停摆。与互锁 2s 分开计——
                                    # 互锁是"该吸的没吸住"硬故障线，本线是
                                    # "吸住了但还软"的等待线
