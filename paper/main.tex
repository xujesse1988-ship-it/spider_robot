...
per-cycle position loss occurs within $100$\,ms of seal rupture. A
...
kinematic and open loop, using no force, tactile, pressure, or current
feedback. In an $n{=}3$ Latin-square study it cuts total slip by $43\%$
...
($83\%$ in the \emph{discovery run}, the earlier run in which we first
observed the effect, which used wider segmentation windows), while
during active venting, before the seal lets go, the body moves less
than $0.15$\,mm. The mechanism is elastic: the attached feet stay put
...
walker derived from an open-source hexapod kit, fitted with
self-designed suction feet. The robot climbs facing up the wall; legs
are named L/R for the left/right side and $1/2/3$ from front to rear,
so L1 and R1 are the uppermost pair and L3/R3 the lowest
(Fig.~\ref{fig:platform}a). Each leg has three position-controlled
joints (coxa/femur/tibia links $43/80/124$\,mm) and ends in a 30\,mm
bellows suction cup. A normally-closed 3-way solenoid valve per foot
connects the cup either to a vacuum reservoir (attach) or to atmosphere
(vent), and a per-foot check valve isolates the cups from one another.
Two diaphragm pumps hold the attached cups between $-60$ and $-75$\,kPa
by bang-bang hysteresis; a cup counts as attached below $-30$\,kPa; a
leg may lift only when the five other cups are attached and deeper than
a $-50$\,kPa gate. The gait moves one leg at a time: at any moment five
feet are attached and at most one is in the air. Eighteen
35\,kg$\cdot$cm position-controlled hobby servos (no torque, force,
tactile, or per-joint current feedback of any kind) are driven by a
Raspberry~Pi~5 through an RP2040 servo board; total mass is
${\sim}2.7$\,kg \TODO{final weigh-in of assembled robot}. All
experiments run on vertical glass with a safety rope. The platform's
...
\emph{vent} (a fixed $[-1,+0.4]$\,s window around the logged
pressure-zero event, containing the seal rupture; we call the slip
accrued in it the \emph{rupture-segment slip}), \emph{swing} (foot in
the air), \emph{press} (touchdown), and \emph{settle}. The pipeline was verified against a manual tape-ruler reading
...
lifted $33$\,mm off the glass, hovers $1$\,s, and is put back at exactly
its former station; the commanded net body motion is zero, so any
systematic displacement must come from elastic strain release and
re-locking. Two controls verify the premises: ink marks on the glass
beside each attached cup do not move (the cups do not slip), and the
body is stationary whenever all six feet are attached (no slow drift;
...
bounces reach $20.9$\,mm (L1) and $19.2$\,mm (R1)---the top pair,
where the body's weight exerts the largest peel moment on the
cups---versus $5.8$\,mm for the softest legs, a first sign that
leg-chain stiffness is strongly unequal.
...
later rounds as loading accumulates.
...
down the wall and gives back its own $\delta_j$ when its turn comes, so
offsets do not diverge; the small per-leg mismatch is cleared when the
foot is placed again at touchdown (this causes a ${\le}5$\,mm command
artifact in the first round only).
...
would pile load onto one leg. Weights change the steady-state operating point, so the
$\delta$ table is calibrated \emph{under} the weight schedule
(Sec.~\ref{sec:protocol}).

\subsection{Implementation and guards}

The method is ${\sim}200$ lines in the gait controller: one new phase
between the lift decision and VENT, one per-tick displacement update,
no new hardware, no sensing, no per-robot tuning beyond the $\delta$
table. Safety guards (none of which fired in any run reported here):
...
uniform shares ($w_i = 1/5$); C: B $+$ rotation-distance weights (table
re-calibrated under the weights). One variable changes per condition.
...
pressure-gated pacing (the next lift waits until the pumps have
switched off at $-75$\,kPa), $15$\,s between rounds, $30$\,s final
settle---so pacing is identical across runs by construction. Battery
$\ge7.9$\,V at each session start; camera untouched within a session;
...
the H2 prediction toward the H1 prediction, and the small paired
difference is itself largely window attribution (a ${\sim}1$\,mm relaxation tail
...
\eqref{eq:redistribution} for the stiffest leg (L1's coefficient is
bounded below at $0.019$)---so a designer who wants software-only
