...
加`--dual-front`可做双前足依次吸墙、四足留地的静态实验，默认压入上限2 mm；
`hold`要求两足连续10秒压力达标，然后逐足释放返回。该模式需防跌承托/安全绳，
不允许抬头或移动中后足，压力达标不代表载荷转移已验证。
双足静态实验通过后，可再加`--pitch-probe`做小幅抬头：每步≤0.5°、硬上限2°，
每次增加前先`hold`，回到0°后才允许释放收腿。首轮只试0°→0.5°→0°；
四只中后足留地，仍需能接住整机的安全绳/承托，没有姿态或接触力反馈。
再加`--shift-probe`可在同一轮验证水平前移：完成抬头往返并回到0°后，
`hold`→`shift 5`→`hold`→`shift 0`→`hold`先试5 mm往返。
通过后逐步前移到30 mm；`shift`是绝对位置，每次变化≤5 mm，继续前移前重新`hold`。
六足指令锚点固定，平移峰值≤5 mm/s；先逐步`shift 0`回到原点才能释放、收腿或再次抬头。
该模式仍不提供中足上墙命令，完整启动、验证及回程命令见上述文档。
针对`prepare R1`抬腿剐蹭，准备/返回轨迹改为先在原落脚点上方抬至墙面目标高度，
再向墙接近，回程先远离墙再下降。`--approach-gap`默认60 mm（原20 mm），
`touch`会推进这一模型间距；执行前实测墙距，新轨迹不能补偿机身下沉。
默认`--speed 20`使运动速度为原来的两倍，含腿部、站起/趴下和俯仰；
`--speed 10`恢复原速度。吸附确认、10秒保持和故障计时不变。
所有模式都只能未吸附冷启动，初始化会排气并断舵机电，不支持挂墙续跑或完整自主翻身。
